...
Keys* g_keys;
static BOOL
	g_isProgramLooping; // Window Creation Loop, For FullScreen/Windowed Toggle																		// Between Fullscreen / Windowed Mode
 ...
 */
RopeSimulation* ropeSimulation = new RopeSimulation(80, // 80 Particles (Masses)
...
void Update(DWORD milliseconds) // Perform Motion Updates Here
{
	...
	for (int a = 0; a < numOfIterations;
	++a) // We Need To Iterate Simulations "numOfIterations" Times
...
LRESULT CALLBACK WindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYUP: // Update Keyboard Buffers For Keys Released
			...
			// Is Key (wParam) In A Valid Range?
		{
			...
			// Set The Selected Key (wParam) To False
			return 0; // Return
		}
		break; // Break
...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
	...
	// g_createFullScreen Is Set To User Default
	while (g_isProgramLooping) // Loop Until WM_QUIT Is Received
	{
		// Create A Window
		window.init.isFullScreen = g_createFullScreen;
		// Set Init Param Of Window Creation To Fullscreen?
		if (CreateWindowGL(&window) == TRUE) // Was Window Creation Successful?
		{
			...
			if (Initialize(&window, &keys) == FALSE) // Call User Intialization
			{
				...
				// Close Window, This Will Handle The Shutdown
			}
					...
					// While The Message Pump Is Active
				{
					...
					if (PeekMessage(&msg, window.hWnd, 0, 0, PM_REMOVE) != 0) {
						...
						if (msg.message != WM_QUIT)
							// Is The Message A WM_QUIT Message?
						{
							DispatchMessage(&msg);
