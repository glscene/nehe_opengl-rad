...
int LoadGLTextures() // Load Bitmaps And Convert To Textures
{
	...
	texture[0] = SOIL_load_OGL_texture("../../Data/Mud.bmp", SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID, SOIL_FLAG_INVERT_Y);
	texture[1] = SOIL_load_OGL_texture("../../Data/Mud.bmp", SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID, SOIL_FLAG_INVERT_Y);
	texture[2] = SOIL_load_OGL_texture("../../Data/Mud.bmp", SOIL_LOAD_AUTO,
...
int WINAPI _tWinMain(HINSTANCE, HINSTANCE, LPTSTR, int) {
	...
	if (!CreateGLWindow("Lionel Brits & NeHe's 3D World Tutorial", 640, 480, 16,
		fullscreen)) {
		...
		else // If there are no messages
		{
			...
			else // Not time to quit, update screen
			{
				...
				if (keys[VK_F1]) // Is F1 neing pressed?
				{
					...
					if (!CreateGLWindow
						("Lionel Brits & NeHe's 3D World Tutorial", 640, 480,
						16, fullscreen)) {
						return 0; // Quit if window was not created
					}
				}
