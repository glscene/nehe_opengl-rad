...
public:
	...
	Vector3D& operator =
		(Vector3D v) // operator= sets values of v to this Vector3D. example: v1 = v2 means that values of v2 are set onto v1
	{
		x = v.x;
		y = v.y;
		z = v.z;
	...
	Vector3D& operator +=
		(Vector3D v) // operator+= is used to add another Vector3D to this Vector3D.
	{
		x += v.x;
		y += v.y;
		z += v.z;
	...
	Vector3D& operator -=
		(Vector3D v) // operator-= is used to subtract another Vector3D from this Vector3D.
	{
		x -= v.x;
		y -= v.y;
		z -= v.z;
	...
	Vector3D& operator *=
		(float value) // operator*= is used to scale this Vector3D by a value.
	{
		x *= value;
		y *= value;
		z *= value;
...
public:
	...
	Mass** masses;
	// masses are held by pointer to pointer. (Here Mass** represents a 1 dimensional array)
		...
		// Constructor creates some masses with mass values m
	{
		...
		for (int a = 0; a < numOfMasses;
		++a) // We will step to every pointer in the array
				masses[a] = new Mass(m);
...
public:
	ConstantVelocity()
		: Simulation(1, 1.0f) // Constructor firstly constructs its super class with 1 mass and 1 kg
	{
...
public:
	...
	{
		for (int a = 0; a < numOfMasses;
		++a) // we will apply force to all masses (actually we have 1 mass, but we can extend it in the future)
				masses[a]->applyForce(gravitation * masses[a]->m);
		// gravitational force is as F = m * g. (mass times the gravitational acceleration)
	}
...
public:
	...
	// the arbitrary constant point that the mass is connected
