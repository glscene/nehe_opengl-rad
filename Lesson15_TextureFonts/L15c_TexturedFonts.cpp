...
GLvoid KillFont(void) // Delete the font
{
...
int LoadGLTextures() // Load Bitmaps And Convert To Textures
{
	...
	texture[0] = SOIL_load_OGL_texture("../../Data/Lights.bmp", SOIL_LOAD_AUTO,
...
int InitGL(void) // All setup for OpenGL goes here
{
	...
	// Enable coloring of material						( ADD )
...
int DrawGLScene(void) // Here's where we do all the drawing
{
	...
	// Position the text
	glTranslatef(1.1f*float(cos(rot / 16.0f)),
		0.8f*float(sin(rot / 20.0f)), -3.0f);
...
*/
	...
	if (!CreateGLWindow("NeHe's Texturemapped Outline Font Tutorial", 640, 480,
		16, fullscreen)) {
	...
	while (!done) // Loop that runs while done = FALSE
	{
		...
		else // If there are no messages
		{
			...
			if (keys[VK_F1]) // Is F1 being pressed?
			{
				...
				if (!CreateGLWindow
					("NeHe's Texturemapped Outline Font Tutorial", 640, 480, 16,
					fullscreen)) {
					return 0; // Quit if window was not created
				}
			}
